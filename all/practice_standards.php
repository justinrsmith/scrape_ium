...
$csvFile = file('csvs/practice_standards.csv');
$data = [];
...
foreach ($data as $row) {
    ...
    $id = wp_insert_post(array(
        'post_title'    => $row[0],
        'post_date'     => $now,
        'post_type'     => 'practice_standards',
        'post_status'   => 'publish',
    ));
    if ($id) {
        var_dump(wp_get_attachment_by_post_name($row[2]));
        echo $row[2];
        update_field( 'field_5af9e4d178aa3', $row[0], $id );
        update_field( 'field_5af9e4e178aa4', $row[1], $id );
        update_field( 'field_5af9e4ee78aa5',  wp_get_attachment_by_post_name($row[2]), $id );
    } else {
        echo 'fail';
    }
}
